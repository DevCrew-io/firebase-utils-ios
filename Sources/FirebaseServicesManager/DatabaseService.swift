...
public class FSMPath {
    public static let firestore = Database.database().reference()
}
...
public class DatabaseService {
    ...
    public func getList<T: DatabaseNode>(ref: DatabaseReference, _ type: T.Type, completion: @escaping(_ result: Result<T?, Error>) -> ()) {
        ref.getData { error, snapshot in
            ...
            let parsedData: [T] = snapshot?.children.compactMap {
                if let dic = 
                do {
...
//        let db = firestore.collection(collection).document(id)
