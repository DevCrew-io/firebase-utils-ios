...
public class DatabaseService {
    ...
    public func observe<T: DatabaseNode>(ref: DatabaseReference, eventType: DataEventType = .value, completion: @escaping (_ result: Result<[T]?, Error>) -> ()) -> UInt {
        return observeRequest(ref: ref, eventType: eventType, completion: completion)
    }
    ...
    private func observeRequest<T: DatabaseNode>(ref: DatabaseReference, eventType: DataEventType = .value, completion: @escaping (_ result: Result<[T]?, Error>) -> ()) -> UInt {
        return ref.observe(eventType) { snapshot in
            if let dataSnapshot = snapshot.children.allObjects as? [DataSnapshot] {
                var resultArray:[T] = []
                for data in dataSnapshot {
                    let nodeId = data.key // Retrieve the node ID/key
